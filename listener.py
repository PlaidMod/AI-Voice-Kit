...
class Listener:
    def __init__(self):
        # Porcupine listens for the custom "Hello Claude" keyword.
        self.porcupine = pvporcupine.create(
            access_key=config.PICOVOICE_ACCESS_KEY,
            keyword_paths=[config.WAKE_KEYWORD_PATH],
        )
        # PvRecorder feeds Porcupine frames of exactly the size it expects.
        # We do NOT start it here. main.py starts the mic only during the two
        # windows where we actually listen (waiting for the wake word, and
        # capturing a question). Stopping it the rest of the time means the
        # greeting/answer audio never gets buffered and mistaken for speech,
        # and a fresh buffer each time avoids stale-audio false triggers.
        self.recorder = PvRecorder(
            frame_length=self.porcupine.frame_length,
            device_index=config.MIC_DEVICE_INDEX,
        )
        self._running = False
        self.frame_seconds = self.porcupine.frame_length / config.SAMPLE_RATE
    ...
    def wait_for_trigger(self, button_event):
        ...
        the button thread in main.py.
        """
        while True:
            frame = self.recorder.read()           # blocks ~32 ms per frame
            if self.porcupine.process(frame) >= 0:  # >= 0 means keyword detected
                return "wake"
        ...
        while True:
            ...
            if not started:
                ...
                if loudness < config.ENERGY_THRESHOLD:
                    ...
                    silent_run = 0
                if silent_run >= silence_limit or len(frames) >= max_frames * self.porcupine.frame_length:
                    break
